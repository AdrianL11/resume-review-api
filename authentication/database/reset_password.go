...
func (s *ResumeAIAuthDBService) ResetPassword(token string, password string) error {

	// Check If Token is Valid
	if !s.ForgotPasswordValidateToken(token) {
		return errors.New("invalid token")
	}

	// Token Valid, Get User ID from Token
	userId, err := s.profileDBService.GetUserIdByForgotPasswordToken(token)
	if err != nil {
		return err
	}
	...
	err = mongodb.UpdateOne(s.serverSettings.DBName, "forgot_passwords", filter, update)
	if err != nil {
		return err
	}
