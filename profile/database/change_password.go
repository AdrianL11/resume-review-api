...
func (s *ProfileDBService) ChangePassword(id primitive.ObjectID, oldPassword string, newPassword string) error {

	// Get Profile from ID
	var oldPasswordDB mongodb.ChangePassword
	filter := bson.D{{"_id", id}}
	err := mongodb.FindOne(s.serverSettings.DBName, "users", filter, &oldPasswordDB)
	if err != nil {
		return err
	}
